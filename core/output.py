...
class CLIOutput(object):
    ...
    def statusReport(self, path, response):
        with self.mutex:
            ...
            # Format message
            contentLength = str(len(response.content))
    ...
    def openPort(self,mes):
        if 'open' in mes:
            message = Fore.GREEN + mes + Style.RESET_ALL
        else:
            message = Fore.BLUE + mes + Style.RESET_ALL
        self.newLine(message)

    def maskOpenPort(self,mes):
        if 'open' in mes:
            message = Fore.GREEN + mes + Style.RESET_ALL
        if 'close' in mes:
            message = Fore.BLUE + mes + Style.RESET_ALL
        if 'filtered' in mes:
            message = Fore.YELLOW + mes + Style.RESET_ALL
        self.newLine(message)
        
    def dataOut(self,mes):
        message = Fore.GREEN + mes + Style.RESET_ALL
        self.newLine(message)
    ...
    def warning(self, reason):
        message = Style.BRIGHT + Fore.MAGENTA + reason + Style.RESET_ALL
        self.newLine(message)
